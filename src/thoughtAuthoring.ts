...
function findFreePosition({ thoughts, viewport, zoom }: FreeThoughtPositionInput) {
  ...
  const candidates = [preferred, ...scanAxis(minX, maxX).flatMap((x) => scanAxis(minY, maxY).map((y) => ({ x, y })))]
    ...
    (currentBest, candidate) => {
      const thoughtClearance = visibleThoughts.reduce(
        (closest, thought) =>
          Math.min(
            closest,
            Math.hypot(candidate.x - thought.x, candidate.y - thought.y) - thought.radius - collisionRadius - 8,
          ),
      ...
      return clearance > currentBest.clearance ||
        (clearance === currentBest.clearance && preferredDistance < currentBest.preferredDistance)
