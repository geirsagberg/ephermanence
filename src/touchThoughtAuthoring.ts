...
export function shouldPreserveNativeLongPressHaptics(navigatorObject: Pick<Navigator, 'userAgent'>) {
  return /Android/.test(navigatorObject.userAgent) && /Firefox\//.test(navigatorObject.userAgent)
}
...
export function composerPositionForKeyboard({
  ...
  const maximumY = Math.max(minimumY, Math.floor(visibleBottom - composerRadius - composerMargin))
