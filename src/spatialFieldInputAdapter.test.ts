...
function createHarness(
  ...
  let onThoughtPointerDown:
    | ((id: string, point: { x: number; y: number }, singular: boolean, pointerId: number) => void)
    | undefined
  let resizeListener = () => {}
...
function pointerEvent(phase: ThoughtControlEvent['phase'], overrides: Partial<ThoughtControlEvent> = {}) {
  const consume = vi.fn()
...
describe('spatial field input adapter', () => {
  ...
  it('does not edit after a double click turns into a drag', async () => {
    ...
    harness.canvas.emit('pointerdown', nativePointer({ clientX: start.x, clientY: start.y }))
    harness.events.emit('pointerup', nativePointer({ clientX: start.x, clientY: start.y }))

    harness.thoughtPointerDown(existing.id, start, false, 1)
    harness.canvas.emit('pointerdown', nativePointer({ clientX: start.x, clientY: start.y }))
    harness.events.emit('pointermove', nativePointer({ clientX: start.x + 20, clientY: start.y }))
    harness.events.emit('pointerup', nativePointer({ clientX: start.x + 20, clientY: start.y }))
