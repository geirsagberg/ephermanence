...
describe('spatial field scene', () => {
  ...
  it('renders Bonds before interactive Thoughts', () => {
    ...
    scene.render(snapshot({ thoughts: [first, second], attachments: [['first', 'second']] }), viewport)
  ...
  it('forwards the pointer identity needed to recognize a long press', () => {
    ...
    expect(onPointerDown).toHaveBeenCalledWith('held', { x: 12, y: 34 }, false, 7, 'touch')
  })
  ...
  it('reuses an unchanged Thought display object across drag frames', () => {
    ...
    scene.render(snapshot({ thoughts: [{ ...thought, x: 40 }], attachments: [] }), viewport)
  ...
  it('reuses a Thought display object while drawing its contact outline behind it', () => {
    ...
    scene.render(snapshot({ thoughts: [thought], attachments: [] }, ['candidate']), viewport)

    expect(thoughts(scene).children[0]).toBe(renderedThought)
    expect(clusterOutline(scene).bounds.minX).toBeCloseTo(-thoughtRadius(thought.text) - 8)
    expect(clusterOutline(scene).bounds.maxX).toBeCloseTo(thoughtRadius(thought.text) + 8)
  })
  ...
  it('raises a bonded Thought while it is grabbed independently', () => {
    ...
    scene.render({ ...snapshot(state), independentlyMovingThoughtIds: ['first'] }, viewport)
  ...
  it('reuses a Bond display object while its endpoint moves', () => {
    ...
    scene.render(snapshot({ thoughts: [first, { ...second, x: 140 }], attachments }), viewport)
  ...
  it('discards distant chunks when the camera moves to another region', () => {
    ...
    expect(ambient(scene).children.every((chunk) => !previousChunks.has(chunk))).toBe(true)
  })
  ...
  it('rebuilds cached chunks when the appearance changes', () => {
    ...
    expect(ambient(scene).children.every((chunk) => !previousChunks.has(chunk))).toBe(true)
    expect(ambient(scene).children.every((chunk) => chunk.children.length === 2)).toBe(true)
  })
})
