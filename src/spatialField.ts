    ...
    dispatch(input) {
      switch (input.type) {
        case 'thought-pointer-down': {
          ...
          const ownedIds = new Set([...drags.values()].flatMap(({ movingIds }) => [...movingIds]))
          drags.set(input.pointerId, {
            ...
            movingIds: independent
              ? new Set([input.id])
              : new Set([...connectedThoughtIds(input.id, state.attachments)].filter((id) => !ownedIds.has(id))),
            independent,
            detachOnTap: input.detachOnTap ?? false,
            raiseOnDragStart: !state.attachments.some(([a, b]) => a === input.id || b === input.id),
          })
          nextDragOrder += 1
        ...
        case 'pointer-move': {
          ...
          const thoughts = translateThoughts(
            dragBegan && drag.raiseOnDragStart ? bringThoughtToFront(state.thoughts, drag.activeId) : state.thoughts,
            drag.movingIds,
            dx / input.zoom,
            dy / input.zoom,
          )
        ...
        case 'pointer-up': {
          ...
          if (drag.distance < 4) {
            if (drag.detachOnTap) {
              ...
              state = {
                ...state,
                attachments: state.attachments.filter(([a, b]) => a !== activeId && b !== activeId),
              }
            } else {
              state = {
                ...state,
            ...
            state = {
              ...state,
              attachments: recalculateAttachments(state.thoughts, state.attachments, drag.movingIds),
            }
          }
          drags.delete(input.pointerId)
        ...
        case 'delete-selection': {
          ...
          state = {
            ...
            attachments: state.attachments.filter(([a, b]) => a !== selectedId && b !== selectedId),
          }
          selectedId = null
          break
        }
        case 'edit-thought': {
          ...
          state = {
            thoughts,
            attachments: recalculateAttachments(thoughts, state.attachments, new Set([input.id])),
          }
          break
        }
        case 'create-thought': {
...
function translateThoughts(thoughts: Thought[], movingIds: Set<string>, dx: number, dy: number) {
  return thoughts.map((thought) =>
    movingIds.has(thought.id) ? { ...thought, x: thought.x + dx, y: thought.y + dy } : thought,
  )
}
  ...
  return [
    ...new Set(
      [...drags.values()]
        ...
        .flatMap(({ movingIds }) => findAttachmentCandidateIds(state.thoughts, state.attachments, movingIds)),
    ),
  ]
}

function transferReleasedDragToOldestAttachedDrag(released: Drag, drags: Map<number, Drag>, attachments: Attachment[]) {
  const recipient = [...drags.values()]
    .filter(({ movingIds }) =>
      attachments.some(
        ([a, b]) => (released.movingIds.has(a) && movingIds.has(b)) || (released.movingIds.has(b) && movingIds.has(a)),
      ),
    )
...
function findAttachmentCandidateIds(thoughts: Thought[], attachments: Attachment[], movedIds: Set<string>) {
...
function findNewAttachments(thoughts: Thought[], attachments: Attachment[], movedIds: Set<string>) {
  ...
  for (let index = 0; index < thoughts.length; index += 1) {
    ...
    for (let targetIndex = index + 1; targetIndex < thoughts.length; targetIndex += 1) {
      ...
      if (aIsMoving === movedIds.has(b.id) || !areTouching(a, b, radii[index], radii[targetIndex])) continue
...
function recalculateAttachments(thoughts: Thought[], attachments: Attachment[], movedIds: Set<string>) {
  const retained = retainAttachments(attachments, movedIds)
