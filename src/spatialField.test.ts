...
describe('spatial field transitions', () => {
  it('selects a clicked thought and brings it to the front', () => {
    const spatialField = field([thought('front', 100), thought('selected', 200), thought('back', 300)])
    ...
    expect(snapshot.state.thoughts.map(({ id }) => id)).toEqual(['front', 'back', 'selected'])
  })

  it('preserves depth on an ordinary pointer down', () => {
    const spatialField = field([thought('pressed', 100), thought('middle', 200), thought('front', 300)])

    startDrag(spatialField, 'pressed')

    expect(spatialField.read().state.thoughts.map(({ id }) => id)).toEqual(['pressed', 'middle', 'front'])
  ...
  it('brings a thought to the front when solo dragging begins', () => {
    ...
    expect(spatialField.read().state.thoughts.map(({ id }) => id)).toEqual(['joined', 'front', 'solo'])
  })
  ...
  it('moves an attached group through a complete drag at the current zoom', () => {
    ...
    expect(snapshot.state.thoughts).toEqual([thought('moving', 110), thought('joined', 210), thought('fixed', 400)])
  ...
  it('moves one thought and replaces only its bonds after a singular drag', () => {
    ...
    expect(snapshot.state.thoughts).toEqual([thought('old', 100), thought('new', 405), thought('moving', 305)])
  ...
  it('exposes a singular grab before movement and clears it on release', () => {
    const spatialField = field([thought('grabbed', 100), thought('joined', 200)], [['grabbed', 'joined']])
  ...
  it('resizes an edited thought and recalculates its bonds', () => {
    const spatialField = createSpatialField({
      thoughts: [{ ...thought('editing', 100), text: 'A'.repeat(100) }, thought('nearby', 270)],
