...
export function getLauncherDragCenter({ launcherCenter, pointer, pointerStart }: LauncherDrag): Position {
  return {
...
export function getLauncherDragUpdate(drag: LauncherDrag): {
  ...
  return {
    ...
    isDrag: Math.hypot(drag.pointer.x - drag.pointerStart.x, drag.pointer.y - drag.pointerStart.y) > 7,
  }
}
