...
import type {
  ThoughtAuthoring,
  ThoughtAuthoringInput,
  ThoughtAuthoringState,
} from './thoughtAuthoring';
...
const sendAuthoringStateAtom = atom<((input: ThoughtAuthoringInput) => void) | null>(
  null,
);

export const ambientBubbleSettingsAtom = atom<AmbientBubbleSettings>(
  defaultAmbientBubbleSettings,
);
...
const applyFieldFrameAtom = atom(
  ...
  (
    ...
    if (update.frame.launchRequests > 0) {
      set(
        launcherRequestStateAtom,
        get(launcherRequestStateAtom) + update.frame.launchRequests,
      );
    }
  },
);
