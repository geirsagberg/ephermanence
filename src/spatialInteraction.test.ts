...
describe('spatial field interaction', () => {
  ...
  it('exposes every Thought split into an independent touch session', () => {
    ...
    expect(spatialInteraction.read().independentlyMovingThoughtIds).toEqual(['second', 'third'])
  })
  ...
  it('gathers one cluster while an unrelated Thought remains held', () => {
    ...
    expect(
      Object.fromEntries(spatialInteraction.read().state.thoughts.map((existing) => [existing.id, existing.x])),
  ...
  it('requests editing when double-clicking a Thought', () => {
    ...
    expect(transition.effects).toEqual([{ type: 'request-edit', thought: existing, screenPosition }])
  })
  ...
  it('loads the durable spatial field before using the fallback', () => {
    ...
    const memory = memoryStorage(JSON.stringify({ thoughts: [storedThought], attachments: [] }))
  ...
  it('commits durable field commands without caller-owned policy', () => {
    const memory = memoryStorage()
    const spatialInteraction = createSpatialInteraction({ thoughts: [], attachments: [] }, memory.storage)
  ...
  it('waits until pointer-up to commit a dragged Thought', () => {
    ...
    const memory = memoryStorage()
    const spatialInteraction = createSpatialInteraction({ thoughts: [existing], attachments: [] }, memory.storage)
    spatialInteraction.dispatch({
      type: 'viewport-resize',
      size: { width: 1000, height: 800 },
    })
