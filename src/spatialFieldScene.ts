...
import type { SpatialInteraction, SpatialInteractionSnapshot } from './spatialInteraction'
...
type ThoughtPointerDown = (id: string, point: Point, singular: boolean, pointerId: number, pointerKind?: string) => void
...
export class SpatialFieldScene extends Container {
  ...
  private readonly fadingThoughts = new Map<string, { bubble: Container; elapsed: number }>()
  private readonly fadingBonds = new Map<string, { graphic: Graphics; elapsed: number }>()
  ...
  presentAuthoring(presentation?: ThoughtAuthoringPresentation) {
    ...
    if (!visual || visual.id !== presentation.id) {
      ...
      const sourceY =
        presentation.position.y - thoughtRise * presentation.elevation.source * presentation.elevation.zoom
    ...
    visual.duration =
      presentation.phase === 'cancel-dismiss' ? thoughtAuthoringDismissDuration : thoughtAuthoringCloseDuration
    visual.toAlpha = presentation.phase === 'cancel-dismiss' ? 0 : thoughtAppearanceStartAlpha
    visual.toScale = presentation.phase === 'cancel-dismiss' ? 0.82 : presentation.closeScale
    visual.toElevation = presentation.phase === 'cancel-dismiss' ? 1 : presentation.elevation.target
    visual.toY =
    ...
    if (presentation.phase !== 'cancel-dismiss' && presentation.text) {
      ...
      const preview = createThoughtLabel(presentation.text, radius, this.colorModeProgress)
  ...
  render(
    ...
    const { camera, state, selectedId, independentlyMovingThoughtIds, attachmentCandidateIds } = snapshot
    ...
    const radii = new Map(state.thoughts.map((thought) => [thought.id, thoughtRadius(thought.text)]))
    ...
    for (const thought of state.thoughts) {
      ...
      const targetElevation = !bondedThoughtIds.has(thought.id) || independentlyMovingThoughts.has(thought.id) ? 1 : 0
  ...
  advanceAnimations(deltaMs: number) {
    if (this.colorModeProgress !== this.colorModeTarget) {
      ...
      this.colorModeProgress = Math.max(
        ...
        Math.min(
          ...
          this.colorModeProgress +
            direction *
              Math.min(deltaMs / colorModeTransitionDuration, Math.abs(this.colorModeTarget - this.colorModeProgress)),
        ),
      )
      this.applyColorMode()
    }
    ...
    if (authoringVisual && authoringVisual.elapsed < authoringVisual.duration) {
      authoringVisual.elapsed = Math.min(authoringVisual.duration, authoringVisual.elapsed + deltaMs)
      ...
      authoringVisual.bubble.alpha = interpolate(authoringVisual.fromAlpha, authoringVisual.toAlpha, eased)
      authoringVisual.bubble.scale.set(interpolate(authoringVisual.fromScale, authoringVisual.toScale, eased))
      authoringVisual.bubble.y = interpolate(authoringVisual.fromY, authoringVisual.toY, eased)
      authoringVisual.elevation = interpolate(authoringVisual.fromElevation, authoringVisual.toElevation, eased)
    ...
    if (this.clusterOutline.alpha !== this.clusterOutlineTargetAlpha) {
      const direction = Math.sign(this.clusterOutlineTargetAlpha - this.clusterOutline.alpha)
    ...
    for (const record of this.thoughtBubbles.values()) {
      if (record.appearance < 1) {
        record.appearance = Math.min(1, record.appearance + deltaMs / thoughtAppearanceDuration)
        applyThoughtAppearance(record.bubble, record.appearance)
      }
      ...
      record.elevation = Math.max(
        ...
        Math.min(
          1,
          record.elevation + direction * Math.min(elevationStep, Math.abs(record.targetElevation - record.elevation)),
        ),
      )
...
function sameThoughtVisual(
  ...
  return record.text === thought.text && record.radius === radius && record.tone === thought.tone
}
...
function sameBondGeometry(left: BondGeometry, right: BondGeometry) {
  return (
    left.from.x === right.from.x && left.from.y === right.from.y && left.to.x === right.to.x && left.to.y === right.to.y
  )
}
...
function drawBond(graphic: Graphics, { from, to }: BondGeometry) {
  return graphic.clear().moveTo(from.x, from.y).lineTo(to.x, to.y).stroke({ color: 0xa6a99e, width: 10, alpha: 0.16 })
}

export type MountedSpatialFieldScene = {
  canvas: HTMLCanvasElement
  screen: { width: number; height: number }
  render: (settings?: AmbientBubbleSettings, hiddenThoughtId?: string, colorMode?: 'light' | 'dark') => void
...
function drawThoughtBody(body: Graphics, radius: number, tone: number, colorModeProgress: number) {
  const thoughtTone = getThoughtTone(tone)
  return body
    .clear()
    .circle(0, 0, radius)
    .fill({
      color: interpolateColor(thoughtTone.canvas, thoughtTone.darkCanvas, colorModeProgress),
...
function applyThoughtLabelColor(label: Text, colorModeProgress: number) {
  label.style.fill = interpolateColor(lightThoughtText, darkThoughtText, colorModeProgress)
}
...
class AmbientBubbleField extends Container {
  ...
  update(bounds: WorldBounds, settings: AmbientBubbleSettings = defaultAmbientBubbleSettings) {
...
function createAmbientChunk(chunkX: number, chunkY: number, settings: AmbientBubbleSettings) {
  ...
  for (let index = 0; index < settings.density; index += 1) {
    ...
    const tone = ambientTones[Math.floor(random(chunkX, chunkY, salt + 4) * ambientTones.length)]
...
function interpolateColor(from: number, to: number, progress: number) {
  const channel = (shift: number) => Math.round(interpolate((from >> shift) & 0xff, (to >> shift) & 0xff, progress))
...
function random(x: number, y: number, salt: number) {
  let value = Math.imul(x, 374_761_393) + Math.imul(y, 668_265_263) + Math.imul(salt, 69_069)
